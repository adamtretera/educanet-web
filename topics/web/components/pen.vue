<template>
  ...
  >
    See the Pen
    <a :href="'https://codepen.io/' + author + '/pen/' + name">Here</a>
  </iframe>
</template>
...
export default {
  ...
  computed: {
    ...
    calculatedTheme(): string | "default" {
      ...
      if (this.isDarkTheme) return "dark";
      return "light";
    },
    url(): string {
      return (
    ...
    queryParams(): string {
      ...
      if (calculatedTheme !== "default") qp.append("theme-id", calculatedTheme);
      return qp.toString();
    },
    shouldDisplay(): boolean {
      if (this.displayWhenInDistance === -1 || this.alreadyDisplayed)
        return true;
